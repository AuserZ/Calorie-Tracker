...
export default function MealCard({ meal, onDelete }: Props) {
  ...
  const time = t
    ...
    : "";
  return (
    <article className="flex gap-3 bg-surface rounded-card border border-line p-3 items-center hover:border-primary/40 transition">
      <Link
        href={`/meal/${meal.id}`}
        className="flex flex-1 min-w-0 gap-3 items-center cursor-pointer"
      >
        ...
        <ChevronRight
          size={18}
          className="text-ink-soft shrink-0 hidden sm:block"
          aria-hidden
        />
      </Link>
      {onDelete && (
